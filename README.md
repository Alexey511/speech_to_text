...
Создание высококачественной системы распознавания русской речи путем дообучения передовых моделей (Whisper, Speech2Text) на официальном датасете Mozilla Common Voice 22.0 (~250 часов качественного русского аудио).
...
- **Поддержка SOTA моделей**:
  ...
  - Custom CNN + Transformer архитектура
...
- **Общий объем:** ~250 часов русского аудио
...
### 3. Custom CNN + Transformer (Experimental)
Собственная модель с нуля для сравнения с SOTA.

- CNN feature extractor (4 Conv1d слоя)
- Transformer encoder с позиционным кодированием
- CTC loss для alignment-free обучения
...
Для удобного сравнения нескольких моделей используйте скрипт `run_experiments.py`:

```bash
# Сравнение Whisper моделей разного размера
python run_experiments.py --experiments whisper_base whisper_small

# Сравнение Whisper и Speech2Text
python run_experiments.py --experiments whisper_small s2t_cross_lingual

# Debug режим (быстрое тестирование всех экспериментов)
python run_experiments.py --debug
```

**Результаты** сохраняются в `experiments/` с автоматической генерацией:
- Сравнительной таблицы (CSV, JSON)
- Графиков обучения для каждой модели
- Агрегированных метрик

### Ручное запуск экспериментов
